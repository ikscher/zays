...
class MooMySQL {
	 ...
	 * @return array
	 */
	function getAll($sql, $noCache = false, $domemcached = false, $memcachedtime = 0) {
		global $nomemcached;
		if($_GET['debug']){
		$time = microtime();
	 ...
	 */
	 ...
	 function insertId() {
		return ($id = mysql_insert_id($this->wdbConn)) >= 0 ? $id : $this->result($this->query("SELECT last_insert_id()"), 0);
	}
