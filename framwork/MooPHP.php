 ...
 * 高经理短信发送接口
 **/
function SendMsg_gao($tel,$content,$type=''){
    $uid = '1371';
    $account='zays';
    $pwd = '888666zaysw';
...
//莫名股份公司
function sendSMS_mm($telphone,$content,$type=''){
    $username='70200573';
	$password=md5('13856900659');
	
	$content = urlencode($content);
    if($type){
        $phone=implode(',',$telphone);
    }else{
        $phone=$telphone;
    }

	
    $url="http://api.momingsms.com/?action=send&username={$username}&password={$password}&phone={$phone}&content={$content}&encode=utf8";
