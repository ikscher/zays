...
if($newcity){
	...
	//$content || die("No such city's data");
		...
		$wind = isset($current->wind_condition) ? $current->wind_condition->attributes() : '';
		...
		//echo $low = $xml->weather->forecast_conditions->low->attributes();
		//echo $high = $xml->weather->forecast_conditions->high->attributes();
		$json="";
		for($i=0;$i<=2;$i++){
			$json.= $xml->weather->forecast_conditions[$i]->condition->attributes()."||";//天气：如晴
			$json.= $xml->weather->forecast_conditions[$i]->icon->attributes()."||"; //天气图标
			//$w="http://www.google.com".$ticon;
			//echo "<img src=".$w.">";
			$json.= $xml->weather->forecast_conditions[$i]->low->attributes()."||";//最底温度
			$json.= $xml->weather->forecast_conditions[$i]->high->attributes()."||";//最高温度
		}
		echo $json;
	}else{echo "100";}
}else{echo "100";}
