...
<form action="" method="get">
  <tr border="2">
     ...
     <select name="choose" id="choose">
	    ...
	    <option value="senduid" <?php if($choose=='senduid') { ?>selected="selected"<?php } ?>>发送鲜花ID</option>
		<option value="receiveuid" <?php if($choose=='receiveuid') { ?>selected="selected"<?php } ?>>接受鲜花ID</option>
...
<table cellspacing='1' cellpadding='3' id='list-table'>
  ...
  <tr id="tr_<?php echo $v['rid'];?>">
    <td align="center">
	<a href="#" class="userinfo" onclick="parent.addTab('查看<?php echo $v['senduid'];?>资料','index.php?action=allmember&h=view_info&uid=<?php echo $v['senduid'];?>','icon')">
    ...
    <td align="center"><?php if(isset($v['receiveime']) && $v['receiveime'] != '0') { ?><?php echo date("Y-m-d H:i:s",$v['receivetime']);?><?php } ?></td>
    <td align="center"><?php if(isset($v['send_del']) && $v['send_del'] == '1') { ?>已删除<?php } else { ?>未删除<?php } ?></td>
    <td align="center"><?php if(isset($v['send_deltime']) && $v['send_deltime'] != '0') { ?><?php echo date("Y-m-d H:i:s",$v['send_deltime']);?><?php } ?></td>
    <td align="center"><?php if(isset($v['receive_del']) && $v['receive_del'] == '1') { ?>已删除<?php } else { ?>未删除<?php } ?></td>
    <td align="center"><?php if(isset($v['receive_deltime']) && $v['receive_deltime'] != '0') { ?><?php echo date("Y-m-d H:i:s",$v['receive_deltime']);?><?php } ?></td>
    <td align="center"><?php if(isset($v['is_server']) && $v['is_server'] == '1') { ?>已删除<?php } else { ?>未删除<?php } ?></td>
    <td align="center">
    <?php if(isset($v['dealstate']) && $v['dealstate'] == '1') { ?>
           已处理
    <?php } else { ?>
    <a href="#" onclick="dealok(<?php echo $v['rid'];?>);">未处理</a>
    <?php } ?>  
    </td>
