        ...
        <form name="frmV" action="#" method="get" >
            <div>
                <?php if(!empty($group)) { ?>
					...
					<span class="gid"><input type="radio"  name="groupid" id="gid<?php echo $k;?>" value="<?php echo $v['id'];?>" <?php if($v['id']==$groupid) { ?>checked="checked"<?php } ?> /><label for="gid<?php echo $k;?>"><?php echo $v['manage_name'];?></label></span>
					<?php }?>
				<?php } ?>
            ...
            <table width="100%" cellspacing='1' cellpadding='1' id='list-table' class ="csstab" style="table-layout:fixed;">
                ...
                <tr>
					<?php for($i=0;$i<count($grade);$i++ ) {?>
						<th>原有</th>
						<th><?php if($i==0) { ?>新分<?php } else { ?>新开<?php } ?></th>
						<th><?php if($i==0) { ?>去电<?php } else { ?>跟进<?php } ?></th>
						<th><?php if($i==0) { ?>删除<?php } else { ?>移出<?php } ?></th>
					<?php } ?>
				 </tr>
					<?php if(count($effectGradeArr)>0) { ?>
                    <?php foreach((array)$effectGradeArr as $key=>$value) {?>
                    <tr >
                        <td align="left" class="cs"><?php echo $value['sid'];?>：<?php echo $value['usercode'];?>号<?php echo $value['username'];?></td>
 

                        <?php if(!empty($value['effectGrade'])) { ?>

                        <?php foreach((array)$value['effectGrade'] as $k__=>$v__) {?>
						<td align="center" data-uid="<?php if(!empty($v__[0])) { ?><?php echo implode(',',array_keys($v__[0]));?><?php } ?>" class="t<?php echo $k__;?>"><a href="#"><?php echo count($v__[0]);?></a></td>
						<td align="center" data-uid="<?php if(!empty($v__[1])) { ?><?php echo implode(',',array_keys($v__[1]));?><?php } ?>" class="t<?php echo $k__;?>"><a href="#"><?php echo count($v__[1]);?></a></td>
                        <td align="center" data-uid="<?php if(!empty($v__[2])) { ?><?php echo implode(',',array_keys($v__[2]));?><?php } ?>" class="t<?php echo $k__;?>"><a href="#"><?php echo count($v__[2]);?></a></td><!--<a href="#" onclick="parent.addTab('查看客服<?php echo $value[1]["sid"];?>的<?php echo $k__-1;?>类当日跟进','index.php?action=allmember&h=class&choose=sid&keyword=<?php echo $value["sid"];?>&effect_grade=<?php echo $k__;?>&end=<?php echo $tEnd;?>&start=<?php echo $tStart;?>&clear=1')" >-->
                        <td align="center" data-uid="<?php if(!empty($v__[3])) { ?><?php echo implode(',',array_keys($v__[3]));?><?php } ?>" class="t<?php echo $k__;?>"><a href="#"><?php echo count($v__[3]);?></a></td>
                        <?php }?>
                        <?php } ?>
