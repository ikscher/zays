...
<input type='hidden' name='clear' value='1' />
  <tr>
  	<td colspan="8" style="text-align:left">
		...
		<?php if(isset($grade) && $grade) { ?>
		   <select name="effect_grade" id="efffect_grade" class="m2 s47">
				...
				<?php foreach((array)$grade as $k=>$v) {?>
				  <option    <?php if($effect_grade == $k ) { ?> selected="selected" value="<?php echo $effect_grade;?> "  <?php } ?>><?php echo $v;?></option>
				<?php }?>
			</select>  
		<?php } ?>
		性别:
		...
		<?php if($GLOBALS['h']=='class' ) { ?>
		  <label for="isControl" ><input type="checkbox" name="isControl" id="isControl" value="yes" <?php if($isControl=="yes") { ?> checked="checked" <?php } ?> />可控</label>
		  <label for="isForcast" ><input type="checkbox" name="isForcast" id="isForcast" value="yes"  <?php if($isForcast=="yes") { ?> checked="checked" <?php } ?>  />预测</label>
		<?php } ?>
...
<table cellspacing='1' cellpadding='3' id='list-table' class ="csstab">
  ...
  <?php foreach((array)$member_list as $member) {?>
   <tr>
	...
	<?php if($usertype==3) { ?><td align="center"><?php if(empty($member['action_time'])) { ?>未使用<?php } else { ?><?php echo date("Y-m-d H:i:s",$member['action_time']);?><?php } ?></td><?php } ?>
   ...
   <?php if($member['checkreason'] == '6') { ?>
    ...
    <?php if(!empty($member['real_lastvisit'])) { ?>
        <?php if((time()-$member['real_lastvisit']<100 ) && $member['real_lastvisit']< time() ) { ?> 
			<span style="color:red">在线&nbsp;&nbsp;&nbsp;<?php echo empty($member['client'])?'':'<img src="templates/images/wap_phone.gif" title="手机wap在线">';?></span>
		<?php } elseif ((time()-$member['real_lastvisit']<24*3600) &&  ($member['real_lastvisit']< time() -100) ) { ?>
