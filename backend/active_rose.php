...
function active_rose_list(){
    ...
    //note 处理搜索鲜花
    if(isset($_GET['action']) && $_GET['action'] == 'active_rose') {
	...
	//note 查询语句
    $sql = "SELECT COUNT(*) num FROM {$GLOBALS['dbTablePre']}service_rose s LEFT JOIN {$GLOBALS['dbTablePre']}members_search m ON s.senduid=m.uid $sql_where";
	$total = $GLOBALS['_MooClass']['MooMySQL']->getOne($sql);

	$sql = "SELECT s.*,m.nickname as send_nickname, m.gender as send_gender  FROM {$GLOBALS['dbTablePre']}service_rose s LEFT JOIN {$GLOBALS['dbTablePre']}members_search m ON s.senduid=m.uid $sql_where ORDER BY rid DESC LIMIT {$offset},{$limit}";
	$user_arr = $GLOBALS['_MooClass']['MooMySQL']->getAll($sql);

//	foreach($user_arr as $k => $v) {
//		//note 查询出发送用户的昵称，性别
//		$sql = "SELECT nickname,gender FROM {$GLOBALS['dbTablePre']}members WHERE uid = '{$v[senduid]}'";
...
//		
//		//note 查询出接受用户的昵称，性别
//		$sql = "SELECT nickname,gender FROM {$GLOBALS['dbTablePre']}members WHERE uid = '{$v[receiveuid]}'";
...
//	}
	...
	require(adminTemplate('active_rose_list'));
