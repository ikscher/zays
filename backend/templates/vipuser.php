...
function vipuser_pay_query(){
	...
	$apply_time1=MooGetGPC('apply_time1','string','G');
	if(!empty($apply_time1)){
	  $startTime =  strtotime($apply_time1);
    ...
    $apply_time2=MooGetGPC('apply_time2','string','G');
	if(!empty($apply_time2)){
	  $endTime=strtotime($apply_time2);
	...
	if($source){
	    $condition[] = " in('{$source}','mb.source')>=0";
		$url .= '&source='.$source;
	}
