...
$sql = sprintf('select * from web_members_search where usertype=1 and s_cid<40 and bgtime>=%s and bgtime<%s limit %s,%s', $fromDT, $toDT, $offset, $limit);
...
function nextImport() {
    if (index < length) {
        var user = m[index];
        var url = baseurl + '?uid=' + user.uid + '&puid=&ip=' + user.ip + '&s_cid=40&website=&keyurl=' + user.keyurl + '&dateline=' + user.dateline + '&gender=' + user.gender + '&phone=' + user.phone + '&birthyear=' + user.birthyear + '&is_phone=' + user.is_phone;
