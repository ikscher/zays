<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>资料设置——真爱一生网</title>
<?php include MooTemplate('system/js_css','public'); ?>
<link href="module/material/templates/<?php echo $GLOBALS['style_name'];?>/material.css" type="text/css" rel="stylesheet" />
<script type="text/javascript" src="public/default/js/sys1.js?v=1"></script>
<!-- <script type="text/javascript" src="./module/register/templates/public/js/syscode.js?v=1"></script> -->

</head>

<body>
<?php include MooTemplate('system/header','public'); ?>
<div class="main">
	<div class="content">
	<!-- <p class="c-title"><span class="f-000"><a href="index.php?n=service">真爱一生首页</a>&nbsp;&gt;&gt;&nbsp;资料设置</span><a href="#"></a></p> -->
	<?php include MooTemplate('public/material_public_left','module'); ?>
	<!--=====左边结束===-->
	<div class="content-right">
		<div class="c-right-content">
			<div class="r-center-tilte">
			<a href="index.php?n=service" class="r-title-black">&lt;&lt;返回我的真爱一生</a>
			<span class="right-title f-ed0a91">资料设置</span>
			</div>
            <form id="form1" name="form1" method="post" action="index.php?n=material&h=life_state">
			<div class="r-center-ccontent">
			<div class="register-step"><p class="step03"><a href="index.php?n=material&h=upinfo">基本资料</a><a href="index.php?n=material&h=upinfo&actio=1">详细资料</a><a href="index.php?n=material&h=upinfo&actio=2" class="in-on">生活态度</a><a href="index.php?n=material&h=upinfo&actio=3">兴趣爱好</a></p><span>填写进度：</span>
			<div class="clear"></div>
			</div>
			<h4>&nbsp;<span class="f-aeaeae">&gt;</span>&nbsp;生活状态：</h4>
			<dl class="c-center-data">				
			<dt>是否吸烟：</dt>
					<dd>
						<p class="all-input">
							 <script>getSelect('','smoking','smoking','<?php echo $userinfo['smoking'];?>','0',isSmoking);</script>
						</p>						
					</dd>
					<dt>是否饮酒：</dt>
					<dd>
						<p class="all-input">
							<script>getSelect('','drinking','drinking','<?php echo $userinfo['drinking'];?>','0',isDrinking);</script>
						</p>						
					</dd>
					<dt>职业类别：</dt>
					<dd>
						<p class="all-input">
							<script>getSelect('','occupation3','occupation3','<?php echo $userinfo['occupation'];?>','0',occupation);</script>
						</p>						
					</dd>
					<dt>购车情况：</dt>
					<dd>
						<p class="all-input">
							<script>getSelect('','vehicle','vehicle','<?php echo $userinfo['vehicle'];?>','0',vehicle);</script>	
						</p>						
					</dd>
					<dt>公司类型：</dt>
					<dd>
						<p class="all-input">
							<script>getSelect('','corptype','corptype','<?php echo $userinfo['corptype'];?>','0',corptype);</script>
						</p>						
					</dd>
					<dt>是否要孩子：</dt>
					<dd>
						<p class="all-input">
							<script>getSelect('','wantchildren3','wantchildren3','<?php echo $userinfo['wantchildren'];?>','0',wantchildren);</script>
						</p>						
					</dd>
					<dt>喜欢的美食：</dt>
					<dd style=" height:auto">
						<ul class="all-input">
                        	
							<script>getCheckbox43rdsok('fondfoods','fondfoods[]',',<?php echo $userinfo['fondfood'];?>,',fondfoods);</script>
							
                        </ul>
                    </dd>
					
					<dt>喜欢的地方：</dt>
					<dd style="height:auto">
						<ul class="all-input">
							
                            <script>getCheckbox43rdsok('fondplaces','fondplaces[]',',<?php echo $userinfo['fondplace'];?>,',fondplaces);</script>
                            
						</ul>						
					</dd>					
			</dl>
			<p class="p-bottom">
            <input type="hidden" name="issubmit" value="提交" />
            <input name="actio" type="hidden" id="actio" value="3" />
            <input name="input" type="submit"  class="btn btn-primary go-on" value="保存并继续"/></p>
		<div class="clear"></div>
			</div>
			<div class="r-center-bottom">
			</div>
			<div class="clear"></div>
            </form>
		</div>
	</div>
		<div class="clear"></div>
	</div>
	
	<!--content end-->
	<?php include MooTemplate('system/footer','public'); ?>
</div><!--main end-->
</body>
</html>
