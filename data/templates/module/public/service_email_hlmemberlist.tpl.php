	...
	<div class="content-right">
		<div class="c-right-content">
			...
			<div class="service-title">
				<ul>
					<li ><a href="#" onclick="javascript:location.href='index.php?n=service&h=message&t=allmessage'" ><span>所有来信</span></a></li>
					<li><a href="#" onclick="javascript:location.href='index.php?n=service&h=message&t=membermessage'" ><span>会员来信</span></a></li>
					<li><a href="#" onclick="javascript:location.href='index.php?n=service&h=message&t=hlmessage'" class="onthis"><span>真爱一生来信</span></a></li>
					<li><a href="#" onclick="javascript:location.href='index.php?n=service&h=message&t=sendmessage'" ><span>我发的邮件</span></a></li>
				</ul>
			<div class="clear"></div>
			</div>
			<form name="delmemberid" action="index.php?n=service&h=message&t=hlmessage" method="post"  onsubmit="return checkPageGo('delmessageid[]')">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="my-email-contenter">
			 <?php $num = 1?>
				...
				<tr class="<?php echo $num % 2 ==0?'wf5':'wf'?>">
				   ...
				   <?php if($memails['s_fromid'] != '0' && $memails['is_server'] != 1) { ?>
						 <?php $fromname = getfromname($memails['s_fromid']);?>
						 <?php if(!$fromname) { ?>
						 <?php $fromname = 'ID '.$memails['s_fromid'];?>
						 <?php } ?>
					<?php } else { ?>
						<?php $fromname = '真爱一生网'?>
					<?php } ?>
					<td width="15%" align="center"> 
					<?php if($memails['s_fromid'] != '0' && $memails['is_server'] != '1') { ?><a class="f30" href="index.php?n=space&h=viewpro&uid=<?php echo $memails['s_fromid'];?>"><?php echo $fromname;?></a>
					<?php } else { ?>
             			<?php echo $fromname;?>
					<?php } ?> </td>
					<td width="58%"> <a class="<?php echo !$memails['s_status']?'fzs':'f30'?>" href="index.php?n=service&&h=message&t=detailhlmembermessage&contentid=<?php echo $memails['s_id'];?>">
	                    <?php if($memails['s_fromid'] != '0') { ?>
	                    	<?php echo htmlspecialchars(MooStrReplace($memails['s_title']))?>
	                    <?php } else { ?>
	                    	<?php echo $memails['s_title'];?>
	                    <?php } ?>
	                    </a></td>
			...
			<?php echo multimail($total,$pagesize,$page,$currenturl2)?>共 <span class="f-b-d73c90"><?php echo $total;?></span> 条消息  转到第
				...
				页</span>
				<input name="delmessage" type="submit" class="btn btn-default" value="删除消息" /><!-- <input onclick="javascript:location.href='index.php?n=service&h=backmessage'" name="" type="button" class="my-e-button" value="备份消息" /> --></p>
				</form>
			</div>
