...
XLScroll.prototype.scroll_switch = function(obj,page) {
	...
	j(this.control_pre).onclick = function() {obj.scroll_switch(obj,page - 1)} ;
	j(this.control_next).onclick = function() {obj.scroll_switch(obj,page + 1)} ;

	if (this.control_show_elm && this.control_show_selectedstyle) {
		j(this.control_show_elm).innerHTML = "" ;
		for (i=1;i<=maxpage;i++) {
			...
			j(this.control_show_elm).appendChild(showli) ;
		}
	}
...
var SwitchPic = function(args) {
	this.bigpiccont = j(args.bigpic) ;
	this.switchnav = j(args.switchnav) ;
	this.selectstyle = args.selectstyle ;
	this.objname = args.objname ;
	this.curpage = 0 ;
	this.autotimeintval = 3000 ;
	var bigpics = j(this.bigpiccont).getElementsByTagName("div");
	...
	var switchnavs = j(this.switchnav).getElementsByTagName("li") ;
