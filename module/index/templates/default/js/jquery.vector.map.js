...
(function( $ ){
    ...
    $.fn.vectorMap = function(options) {
        var defaultParams = {
			map: 'world_en',
			backgroundColor: '#10FFFF',//背景色
            color: '#ffffff',
            hoverColor: '#00CCFF',//鼠标放上去颜色
    ...
    var WorldMap = function(params) {
		...
		for(var key in mapData.pathes) {
			...
			path.id = 'jvectormap'+map.index+'_'+mapData.pathes[key].id+'_'+key;
		...
		$(params.container).delegate(this.canvas.mode == 'svg' ? 'path' : 'shape', 'click', function(e){
			var path = e.target;
			var o=e.target.id.split('_');
			var code = o.pop();
			$(params.container).trigger('regionClick.jvectormap', [code]);
			var province_code=o.pop();
			window.location.href="index.php?n=search&h=quick&search_type=1&gender=1&age_start=21&age_end=45&workprovince="+province_code+"&photo=1&quick_search=search";
		});
