...
function will_expire_vip(){
	...
	//将过期vip会员自动转为普通会员
	$current_time = time();
	$sql = "select group_concat(uid) uid_list from {$dbTablePre}members_search WHERE endtime<'{$current_time}' AND s_cid IN(20,30)";
...
//	var_dump($expire_user);
	...
	//转为普通会员
	$sql = "UPDATE {$dbTablePre}members_search SET s_cid=40 WHERE endtime<'{$current_time}' AND s_cid IN(20,30) and usertype='1'";
	$GLOBALS['_MooClass']['MooMySQL']->query($sql);
	$sql2 = "select uid from {$dbTablePre}members_search where endtime<'{$current_time}' AND s_cid IN(20,30) and usertype='1'";
