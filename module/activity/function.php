...
function show_total($stat = '1',$userid) {
	...
	//黑名单
	if($stat=='11'){
		$query = $_MooClass['MooMySQL']->getOne("select count(1) as c FROM {$dbTablePre}screen WHERE uid = '$userid'");
		$total = $query['c'];
	}
	return $total;
}
 ...
 * @param integer $gender 显示性别,如男，女
 * @param integer $stat 显示哪一种性别表示，如女，女士 ...
...
function service_user3($id) {
	...
	if(MOOPHP_ALLOW_FASTDB){
		$user_s = MooFastdbGet('members_search','uid',$id);
		$user_b = MooFastdbGet('members_base','uid',$id);
		$user = array_merge($user_s,$user_b);
	}else{
		$user = $_MooClass['MooMySQL']->getOne("SELECT * FROM {$dbTablePre}members_search,{$dbTablePre}members_base WHERE uid = '$id' and {$dbTablePre}members_search.uid={$dbTablePre}members_base.uid");
	}
	return $user;
}
...
function leer_send_user1($id) {
	...
	//if(MOOPHP_ALLOW_FASTDB){
	//	$user = MooFastdbGet('members','uid',$id);
	//}else{
		$user = $_MooClass['MooMySQL']->getOne("SELECT * FROM {$dbTablePre}members_search,{$dbTablePre}members_base WHERE uid = '$id' and {$dbTablePre}members_search.uid={$dbTablePre}members_base.uid");
	//}
...
function leer_send_user2($id) {
	...
	if(MOOPHP_ALLOW_FASTDB){
		$user = MooFastdbGet('members_choice','uid',$id);
	}else{
		$user = $_MooClass['MooMySQL']->getOne("SELECT * FROM {$dbTablePre}members_choice  WHERE uid = '$id'");
	}
	return $user;
}
...
function getfromname($id) {
	...
	if(MOOPHP_ALLOW_FASTDB){
		...
		$user = $_MooClass['MooMySQL']->getOne("SELECT nickname FROM {$dbTablePre}members_search WHERE uid = '$id'");
	}
...
/*
function get_pic( $userCode, $filename ){
