...
function member_email_content($contentid) {
	...
	//note 获得上一封，下一封邮件
	$advance_member_arr = $_MooClass['MooMySQL']->getAll("SELECT s_id FROM {$dbTablePre}services WHERE s_uid = '$userid' AND  flag = '1' and s_uid_del='0' order by s_time desc ");
	...
	//note 发件人的昵称或ID
	$from_name = $_MooClass['MooMySQL']->getOne("select uid,nickname from {$dbTablePre}members_search where uid='".$memail['s_fromid']."'",true);
...
function hl_member_email_content($contentid) {
	...
	//note 显示查询的内容	
	$memail = $_MooClass['MooMySQL']->getOne("SELECT * FROM {$dbTablePre}services WHERE s_id = '$contentid' and s_uid_del='0'",true);
...
function send_member_email_content($contentid) {
	...
	if(!$sendname){
		$send_name = $_MooClass['MooMySQL']->getOne("select uid,nickname from {$dbTablePre}members_search where uid='".$memail['s_uid']."'",true);
...
function to_send_message(){
	global $_MooClass,$dbTablePre,$userid,$pagesize,$user_arr,$timestamp;
	//print_r($_POST);
	...
	if($mid && $sid && $s_title && $s_content){
	   ...
	   $res= $_MooClass['MooMySQL']->getOne("select s.telphone,b.is_phone,s.username from {$dbTablePre}members_search s left join {$dbTablePre}members_base on s.uid=b.uid where s.uid='$sid'");
	   ...
	   if($interval_time > $week_time){//不活跃用户每周发一条短信
	   		...
	   		$cos = $_MooClass['MooMySQL']->getOne("select count(*) as c from {$dbTablePre}today_send where uid='$sid' and sendtime>='$date1' and sendtime<='$date2'");
	    	if($cos['c']<=1){
			  //fanglin暂时屏蔽
...
//	$total = $_MooClass['MooMySQL']->numRows($query);
	$ret_count = $_MooClass['MooMySQL']->getOne("SELECT count(*) as c FROM {$dbTablePre}services WHERE s_uid = '$userid' and flag = '1' and s_uid_del='0'");
			...
			if($m_level != 1){ //不是高级会员
				if($user_arr['gender'] == 0){//男性
					if(!checkIsMobileCertical($userid)){ //没有通过了手机验证
						...
						if(!checkIsOver($userid)){ //过了试用期
							$sql = "select count(*) as c from " . $dbTablePre . "services where s_status = '1'  and s_uid = " . $userid . " and read_time = '" . date("Y-m-d") . "'";
							$arr = $_MooClass['MooMySQL']->getOne($sql);
							$today_read_count = $arr['c'];
							if($today_read_count > 5){
...
function to_send_hn_message(){
	...
	if($s_title && $s_content){
		...
		//note 添回复的消息、添加发送的消息
		$remark_title = "{$userid}会员给您发消息了，请速查看。";
		$awoketime = time()+600;
		...
		$sql = "INSERT INTO {$GLOBALS['dbTablePre']}services(s_cid,s_uid,s_fromid,s_title,s_content,s_time,is_server,sid,flag)
				VALUES(50,{$userid},'{$sid}','真爱一生消息','您好！您的消息真爱一生已经收到，我们将在一个工作日内解决！4006780405','{$GLOBALS['timestamp']}','1','{$sid}','1')";
		$ret = $GLOBALS['_MooClass']['MooMySQL']->query($sql);
		//短信回复提醒
		Push_message_intab($userid,$user['telphone'],"邮件回复","您的消息真爱一生已经收到，我们将在一个工作日内解决！","system");
