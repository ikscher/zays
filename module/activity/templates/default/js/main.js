var number_of_pictures = 90; 
var root_img_src='module/service/templates/default/images/album/';
function show_pictures () {
	var while_pictures = 1;
	while(while_pictures <= number_of_pictures) {
		var new_image = new Element('img', {
			'src': root_img_src+'thumbs/' + while_pictures + '.jpg',
		...
		var preload_image = new Element('img', {
			'src': root_img_src+'pictures/' + while_pictures + '.jpg',
			'class': 'hide'
...
window.addEvent('domready', function() {	
	...
	if(rows>5) {
		...
		$('up').addEvent('click', function(event){
			if(!$('up').hasClass('deactivated')) {
				vertical_moves--;
				$('down').erase('class');
				$('inside').tween('margin-top', '-'+ (120 * vertical_moves) +'px');
				if (vertical_moves==0) {
	...
	$('left').addEvent('click', function(){
		if(!$('left').hasClass('deactivated')) {
			...
			$('big_picture').set('src', root_img_src+'pictures/' + current_id + '.jpg');
			$('big_picture').fade(1);
