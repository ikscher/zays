...
foreach($collection_data_array as $collection_data){
    ...
    if(!empty($uid)){
		...
		$certifications_data=array('sms'=>1,'telphone'=>1,'email'=>'yes','identity_check'=>3);
        ...
        foreach($image_array as $key=>$value){
			...
			//$imgurl2 = $files['path'].$files['name']."_nowater.".$files['extension'];
			//@copy($imgurl,$imgurl2);//拷贝一张无水印图片（用于形象照）
			$pic_name = $files['name'].".".$files['extension'];
			//$pic_name2 = $files['name']."_nowater.".$files['extension'];
            //将第一张图片的大小控制成我们需要的大小
            ...
            unset($off_wh);
            $big_path=$files['path'];
            //$big_name=$files['name'].'.'.$files['extension'];
			...
			$userimage_path="data/upload/userimg/".$first_dir."/".$secend_dir."/".$third_dir."/".$forth_dir."/";
			...
			!is_dir($userimage_path) && mkdir($userimage_path,0777,true);
			$userimage_big_filename=3*$uid.'_big.jpg';
			$userimage_mid_filename=3*$uid.'_mid.jpg';
			$userimage_medium_filename=3*$uid.'_medium.jpg';
			$userimage_com_filename=3*$uid.'_com.jpg';
			$userimage_small_filename=3*$uid.'_small.jpg';
            ...
            $image->config(array('thumbDir'=>$userimage_path,'thumbStatus'=>'1','saveType'=>'0','thumbName' => $userimage_mid_filename,'waterMarkMinWidth' => '', 'waterMarkMinHeight' => '', 'waterMarkStatus' => 9));
			$image->thumb(171,212, $imgurl);
            $image->waterMark();
			
			$image->config(array('thumbDir'=>$userimage_path,'thumbStatus'=>'1','saveType'=>'0','thumbName' => $userimage_big_filename,'waterMarkMinWidth' => '', 'waterMarkMinHeight' => '', 'waterMarkStatus' => 9));
			$image->thumb(320,400, $imgurl);
            $image->waterMark();
			
			$image->config(array('thumbDir'=>$userimage_path,'thumbStatus'=>'1','saveType'=>'0','thumbName' => $userimage_medium_filename,'waterMarkMinWidth' => '', 'waterMarkMinHeight' => '', 'waterMarkStatus' => 9));
			$image->thumb(100,125, $imgurl);
            $image->waterMark();
			
			$image->config(array('thumbDir'=>$userimage_path,'thumbStatus'=>'1','saveType'=>'0','thumbName' => $userimage_com_filename,'waterMarkMinWidth' => '', 'waterMarkMinHeight' => '', 'waterMarkStatus' => 9));
			$image->thumb(110,138, $imgurl);
            $image->waterMark();
			
			$image->config(array('thumbDir'=>$userimage_path,'thumbStatus'=>'1','saveType'=>'0','thumbName' => $userimage_small_filename,'waterMarkMinWidth' => '', 'waterMarkMinHeight' => '', 'waterMarkStatus' => 9));
			$image->thumb(60,75, $imgurl);
            $image->waterMark();
